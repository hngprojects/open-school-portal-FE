...
import PasswordCreateSuccess from "./password-create-success"
...
const PasswordCreate = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setTokenError("The create link is invalid or expired. Please request a new one.")
      return
    }
    ...
    try {
      ...
      console.error("Password create failed:", err)
      if (err instanceof Error) {
        setTokenError(err.message)
      } else {
        setTokenError(
          "An unexpected error occured while creating your password. Please try again later"
        )
      }
    } finally {
