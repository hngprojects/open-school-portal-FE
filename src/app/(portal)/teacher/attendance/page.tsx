...
import { useGetClassStudents } from "../../admin/class-management/_hooks/use-classes"
import {
  AttendanceAPI,
...
const AttendancePage: React.FC<AttendancePageProps> = ({ classId }) => {
  ...
  const { data: students, isLoading: studentsLoading } = useGetClassStudents(classId)

  // Initialize attendance state only once using useMemo
  const initialAttendanceState = useMemo(() => {
    const state: AttendanceState = {}
    students?.forEach((s) => {
      state[s.student_id] = false
    })
    return state
  }, [students])
  ...
  // Update attendance state if students change
  React.useEffect(() => {
  ...
  const studentList: Student[] =
    students?.map((s) => ({
      id: s.student_id,
      name: s.name,
      present: attendanceState[s.student_id] || false,
    })) || []
  ...
  //   studentList.forEach((s) => (updated[s.id] = true))
  ...
  //   const attendanceRecords: AttendanceRecord[] = studentList.map((s) => ({
  //     student_id: s.id,
      ...
      attendance_records: [
        {
          ...
          status: attendanceState[studentId] ? "PRESENT" : "ABSENT",
        },
      ],
    }
    mutation.mutate(payload)
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="space-y-4">
        {studentList.map((student) => (
          <StudentRow
            ...
            submitStudent={() => handleSubmitStudent(student.id)} // Optional per-student submit
          />
        ))}
      </div>
      ...
      <div className="text-center">
        <h4>You have not been assigned to a class</h4>
      </div>
...
// import { useGetStudents } from "../_hooks/use-results"
// import {
//   AttendanceAPI,
//   SubmitAttendancePayload,
//   AttendanceRecord,
...
//   // Initialize attendance state only once
//   // useEffect(() => {
//   //   if (!students) return
...
//   // Map User -> Student for StudentRow
//   const studentList: Student[] =
//     students?.map((s) => ({
//       id: s.id,
...
//     const payload: SubmitAttendancePayload = {
//       class_id: "324ea7ed-2655-4188-a0e3-80b0682525af",
//       // class_id: classId,
//       date: selectedDate,
//       attendance_records: attendanceRecords,
//     }
...
// //   // Submit attendance mutation
// //   const mutation: UseMutationResult<AttendanceResponse, Error, SubmitAttendancePayload> =
// //     useMutation({
// //       mutationFn: (payload: SubmitAttendancePayload) =>
// //         AttendanceAPI.markDailyAttendance(payload),
// //       onSuccess: () => setModalOpen(true),
// //       onError: (err: Error) => alert(err.message || "Failed to submit attendance"),
// //     })
